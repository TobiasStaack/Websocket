...
websocket_on_open( void* ctx, const int fd, const char* addr )
{
    ...
    const char* const payload2 = "hello world";

    c_ws_frame frame( opcode_text );
    frame.push( reinterpret_cast< const unsigned char* >( payload2 ), std::strlen( payload2 ) );
    ws->emit( fd, &frame );
}
...
#ifdef WEBSOCKET_EXAMPLE_ENDPOINT_SERVER
    if ( websocket_bind( ctx, "localhost", "4433", 0 ) == status_error )
    {
        websocket_destroy( ctx );
        ws_settings_destroy( &settings );
        return 1;
...
#elif WEBSOCKET_EXAMPLE_ENDPOINT_CLIENT
    if ( websocket_open( ctx, "localhost", "4433", 0 ) == status_error )
    {
        websocket_destroy( ctx );
        ws_settings_destroy( &settings );
        return 1;
...
#ifdef WEBSOCKET_EXAMPLE_ENDPOINT_SERVER
    if ( ws.bind( "localhost", "4433", 0 ) == e_ws_status::status_error )
    {
        ws_settings_destroy( &settings );
        return 1;
    }
#elif WEBSOCKET_EXAMPLE_ENDPOINT_CLIENT
    if ( ws.open( "localhost", "4433", 0 ) == e_ws_status::status_error )
    {
        ws_settings_destroy( &settings );
        return 1;
    }
